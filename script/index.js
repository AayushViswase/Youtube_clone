...
let getPopularData = async () => {
    let url = 'https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2CcontentDetails%2Cstatistics&chart=mostPopular&maxResults=20&regionCode=IN&key=AIzaSyAwVqdAh0dPDBJq7MVImOs8JJv7Nd9qZMc'
...
let getSearchData = async () => {
    ...
    if (keyword.length == 0) {
        ...
        let url = `https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=20&q=${keyword}&key=AIzaSyAwVqdAh0dPDBJq7MVImOs8JJv7Nd9qZMc`;
        let res = await fetch(url);
        data = await res.json();
        // console.log(data)
        append(data.items)
